...
# 写入 Params!B12，表示整个逐车流程的全局总预算。
# 求解时会按“第 k 辆车预算 = (总预算 - 已用时间) / 剩余车辆数”动态分配。
# 若不想改 input.xlsx 里的 B12，可设为 None。
GLOBAL_TIME_LIMIT_SEC = 300

# 预留接口：给“每个车次的单车 MIP”再加一个附加上限。
# 默认不启用，只使用 GLOBAL_TIME_LIMIT_SEC 的动态平均分配策略。
# 如需让任何单车最多 300 秒，可改为：VEHICLE_TIME_LIMIT_SEC = 300
# 实际单车时限 = min(动态平均预算, VEHICLE_TIME_LIMIT_SEC)。
...
def main():
    ...
    print(f"[info] 全局总预算 Params!B12 = {GLOBAL_TIME_LIMIT_SEC}")
    print("[info] 单车动态预算策略：第 k 辆车预算 = (总预算 - 已用时间) / 剩余车辆数")
    print(f"[info] 单车附加上限接口 VEHICLE_TIME_LIMIT_SEC = {VEHICLE_TIME_LIMIT_SEC}")
