...
# 预留接口：控制逐车 warmstart 阶段的总时间上限，对应 Excel Params!B12。
# 默认 None 表示不覆盖基础 input.xlsx 里的 B12。
# 例如想把逐车 warmstart 阶段总时间也设为 2400 秒，可改为：WARMSTART_TIME_LIMIT_SEC = 2400
...
def main():
    ...
    print(f"[info] 逐车 warmstart 阶段 B12 覆盖 = {WARMSTART_TIME_LIMIT_SEC}")
