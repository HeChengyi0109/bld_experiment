...
def _solve_single_vehicle_mip(data: ModelData):
    ...
    _configure_model_params(
        model,
        data,
        apply_gap_limits=APPLY_WARMSTART_VEHICLE_GAP_LIMITS,
    )
    ...
    if not APPLY_WARMSTART_VEHICLE_GAP_LIMITS:
        print("[warmstart-time] 已禁用单车 run_epgap / run_epagap 提前停机；本车仅受 time limit 或最优性证明等 SCIP 自然终止条件影响。")
    ...
    if FILL_WARMSTART_VEHICLE_TIME_AFTER_EARLY_STOP and assigned_time_limit is not None:
        sleep_sec = assigned_time_limit - runtime_sec
        if sleep_sec > 1e-6:
            print(f"[warmstart-time] SCIP 提前结束，sleep {sleep_sec:.3f}s 以补足本车预算；注意这段时间不继续优化。")
...
def _resolve_single_vehicle_time_limit(
    ...
    """
    计算顺序 warmstart 阶段当前单车子问题的 SCIP 时间上限。

    强制时间分配策略：
    - 若 Params!B12 > 0，则第 k 辆车开始时使用
      (warmstart 总预算 - 已用时间) / 剩余车辆数；
...
def _build_sequential_warmstart_solution(data: ModelData, vehicle_time_limit: float | None = None):
    """按车辆逐车求解，构造一个可注入整体模型的完整可行解。

    时间分配策略：若 Params!B12 > 0，第 k 辆车开始时把
    “warmstart 剩余总时间 / 剩余车辆数”作为该车 SCIP time limit。
    """
    ...
    for vehicle_id in range(1, total_vehicle_num + 1):
        ...
        if total_time_limit > 0:
            print(
                f"[warmstart-time] 总预算={total_time_limit:.3f}s，已用={total_runtime:.3f}s，"
            ...
            print(f"[warmstart-time] 未设置 warmstart 总预算，启用单车附加上限：本车 SCIP time limit={single_vehicle_time_limit:.3f}s")
        else:
            print("[warmstart-time] 未设置 warmstart 总预算，也未启用单车附加上限；本车不设置 SCIP time limit。")
...
def solve_benders_model(
    ...
    计时逻辑：
    - warmstart 阶段：使用 data.run_tilim 作为逐车阶段总预算，并按
      “剩余总时间 / 剩余车辆数”动态分给每辆车；
    ...
    print(f"[debug] warmstart_vehicle_time_limit = {vehicle_time_limit}")
    ...
    if use_warmstart:
        heuristic_solution, heuristic_runtime, _ = _build_sequential_warmstart_solution(
            data,
            vehicle_time_limit=vehicle_time_limit,
        )
        if heuristic_solution is None:
