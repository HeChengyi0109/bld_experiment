...
# 主 MIP 阶段时间控制
# ==========================
# 这里控制的是：hard-greedy 初始解注入 SCIP 之后，model.optimize() 的运行时间上限。
# warm-start/greedy 构造初始解的耗时不计入这个 SCIP 主求解时间。
# 设为 None 时，恢复使用 input.xlsx -> Params!B12(data.run_tilim)。
FORCE_MAIN_MIP_TIME_LIMIT_SEC = 300.0

# 若想像实验版一样尽量跑满上面的 time limit，把 gap 限制关掉。
# 注意：即使关闭 gap，SCIP 若提前证明 optimal/infeasible，仍会提前停止，这是求解器正常行为。
...
def add_hard_greedy_warm_start(model, data: ModelData, solution: Solution, active_vk: Set[Tuple[int, int]]) -> bool:
    """
    将 hard-greedy 解作为 SCIP 的初始解注入。
    - x 按 solution.V_IDs / B_IDs 赋值；
    - delta 按 active_vk 赋值，其中可包含为满足 B_LB 而补上的空批次。
    返回值为 SCIP 是否接受该初始解。
    """
    x = model.data["x"]
    delta = model.data["delta"]

    assigned_triplets: Set[Tuple[int, int, int]] = set()
    for i in data.I:
        v_id = int(solution.V_IDs[i])
        b_id = int(solution.B_IDs[i])
        if v_id > 0 and b_id > 0:
            v = v_id - 1
            k = b_id - 1
            if v in data.V and k in data.K:
                assigned_triplets.add((i, v, k))
    ...
    try:
        ...
        for i in data.I:
            for v in data.V:
                for k in data.K:
                    model.setSolVal(sol, x[i, v, k], 1.0 if (i, v, k) in assigned_triplets else 0.0)

        for v in data.V:
            for k in data.K:
                model.setSolVal(sol, delta[v, k], 1.0 if (v, k) in active_vk else 0.0)

        accepted = model.addSol(sol)
        print(f"[warm-start] hard-greedy 初始解注入完成，SCIP accepted={accepted}")
        return bool(accepted)
    except Exception as e:
        print(f"[warm-start][warn] hard-greedy 初始解注入失败：{e}")
        return False
...
def solve_benders_model(
    ...
    input_file: str,
    output_file: str,
    main_time_limit_sec: Optional[float] = None,
):
    """
    兼容原 main.py 的调用名：仍叫 solve_benders_model，
    但内部为“直接用 SCIP 求解单体 MIP”。
    在正式求解前，先尝试将 hard-greedy 解注入为初始解。
    """
    ...
    if APPLY_MAIN_MIP_GAP_LIMITS:
        ...
        print("[time-limit]主 MIP 阶段已关闭 gap/absgap 提前停止，仅保留 SCIP 时间上限和证明最优等自然停止条件。")

    configured_time_limit = getattr(data, "run_tilim", 0)
    if main_time_limit_sec is not None and float(main_time_limit_sec) > 0:
        effective_time_limit = float(main_time_limit_sec)
        time_limit_source = "命令行 --main-time-limit"
    elif FORCE_MAIN_MIP_TIME_LIMIT_SEC is not None and float(FORCE_MAIN_MIP_TIME_LIMIT_SEC) > 0:
        effective_time_limit = float(FORCE_MAIN_MIP_TIME_LIMIT_SEC)
        time_limit_source = "bld_solver.py::FORCE_MAIN_MIP_TIME_LIMIT_SEC"
    else:
        effective_time_limit = float(configured_time_limit) if configured_time_limit else 0.0
        time_limit_source = "input.xlsx::Params!B12"

    if effective_time_limit > 0:
        model.setParam("limits/time", effective_time_limit)
        print(
            f"[time-limit]SCIP 主求解时间上限：{effective_time_limit:.1f} 秒 "
            f"({effective_time_limit / 60.0:.2f} 分钟)，来源：{time_limit_source}"
        )
    else:
        print("[time-limit][warn]未设置 SCIP 主求解时间上限。")

    model.setParam("display/verblevel", 4)

    print("开始构造 hard-greedy 初始解 ...")
    warm_t0 = time.time()
    greedy_solution, greedy_info = construct_hard_greedy_solution(data)
    warm_runtime_sec = time.time() - warm_t0
    if greedy_solution.status == "heuristic" and greedy_info.get("active_vk") is not None:
        print(
            f"[warm-start] hard-greedy 构造成功：非空批次={len(greedy_info['used_vk_nonempty'])}，"
            f"补空批次={greedy_info['empty_delta_cnt']}"
        )
        accepted = add_hard_greedy_warm_start(model, data, greedy_solution, greedy_info["active_vk"])
        print(
            f"[warm-start]hard-greedy 构造/注入阶段耗时：{warm_runtime_sec:.3f} 秒；"
            "该耗时不计入 SCIP 主求解时间上限。"
        )
        print(f"[warm-start]初始解是否被 SCIP 接受：{accepted}")
    else:
        print(f"[warm-start][warn] hard-greedy 未构造出完整可行解，本次 SCIP 不使用初始解。原因：{greedy_info.get('reason', '未知原因')}")
        print(
            f"[warm-start]hard-greedy 构造阶段耗时：{warm_runtime_sec:.3f} 秒；"
            "该耗时不计入 SCIP 主求解时间上限。"
        )
